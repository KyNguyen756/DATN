...
import {
  ...
  ChevronRight, Bell, Bus,
} from 'lucide-react';

const navItems = [
  { to: '/admin', label: 'Tổng quan', icon: LayoutDashboard },
  { to: '/admin/routes', label: 'Tuyến & Lịch trình', icon: Route },
  { to: '/admin/vehicles', label: 'Xe & Tài xế', icon: Car },
  { to: '/admin/tickets', label: 'Vé & Đặt chỗ', icon: Ticket },
  { to: '/admin/customers', label: 'Khách hàng & NV', icon: Users },
  { to: '/admin/promotions', label: 'Khuyến mãi & Giá', icon: Tag },
      ...
      <aside style={{
        ...
        <div style={{ padding: '12px 10px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <Link to="/" style={{
            ...
            borderRadius: '10px', color: '#64748B', textDecoration: 'none', fontSize: '13px',
            transition: 'all 0.2s',
          }}>
            <LogOut size={17} style={{ flexShrink: 0 }} />
            {!collapsed && 'Thoát admin'}
          </Link>
        </div>
      </aside>
        ...
        <header style={{
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--gray-800)' }}>Quản trị viên</div>
                <div style={{ fontSize: '11px', color: 'var(--gray-400)' }}>Super Admin</div>
              </div>
            </div>
          </div>
        </header>
