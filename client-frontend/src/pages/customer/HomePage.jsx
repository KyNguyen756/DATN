...
const promos = [
  { label: 'Giảm 20%', title: 'Tuyến TP.HCM - Đà Lạt', desc: 'Áp dụng đến 30/3/2025', code: 'DALATXUAN', color: '#FF6B35' },
  { label: 'Flash Sale', title: 'Vé giường nằm VIP', desc: 'Chỉ còn 10 ghế cuối', code: 'VIPSALE', color: '#8B5CF6' },
  { label: 'Ưu đãi mới', title: 'Tuyến Hà Nội - HCM', desc: 'Đặt trước 7 ngày giảm 15%', code: 'EARLY15', color: '#14B8A6' },
];
              ...
              <div key={i} className="card card-hover" style={{ padding: '20px', cursor: 'pointer' }}
                ...
                <div className="flex items-center justify-between" style={{ marginBottom: '14px' }}>
                  <div className="flex items-center gap-2">
                    <span style={{ fontWeight: '700', fontSize: '15px', color: 'var(--gray-900)' }}>{r.from}</span>
                    <ArrowRight size={16} color="var(--primary)" />
                    <span style={{ fontWeight: '700', fontSize: '15px', color: 'var(--gray-900)' }}>{r.to}</span>
                  </div>
                  <span className="badge badge-primary">{r.trips} chuyến</span>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <span style={{ fontSize: '12px', color: 'var(--gray-500)' }}>Từ </span>
                    <span style={{ fontSize: '18px', fontWeight: '800', color: 'var(--primary)' }}>{r.price}đ</span>
                  </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
            {promos.map((p, i) => (
              <div key={i} style={{
                ...
                <div style={{ background: p.color, padding: '24px', color: 'white' }}>
                  <span style={{
                    background: 'rgba(255,255,255,0.2)', padding: '4px 12px',
                    borderRadius: '20px', fontSize: '12px', fontWeight: '700',
                    display: 'inline-block', marginBottom: '12px',
                  }}>{p.label}</span>
                  <h3 style={{ fontSize: '18px', fontWeight: '800', marginBottom: '6px' }}>{p.title}</h3>
                  <p style={{ opacity: 0.85, fontSize: '13px' }}>{p.desc}</p>
                </div>
                <div style={{ background: 'white', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <button className="btn btn-primary btn-sm" onClick={() => navigate('/search')}>Đặt ngay</button>
                </div>
              </div>
            ))}
          </div>
